...
use smoltcp::{
    iface::{Config as IfaceConfig, Interface as SmolIface, SocketHandle, SocketSet, SocketStorage},
    socket::dhcpv4,
...
static SOCKETS: StaticCell<[SocketStorage<'static>; 3]> = StaticCell::new();
...
/// The scry HTTP client's own socket. RX is deliberately fat: /screen streams
/// 153,600 B and a small window turns the blit into a stall-per-strip.
static HTTP_RX: StaticCell<[u8; 8192]> = StaticCell::new();
...
struct Live {
    ...
    mqtt: mqtt::Client,
    /// Handle for the scry client's socket (see `with_stack`).
    http: SocketHandle,
}
...
pub fn init(
    ...
    let mut sockets = SocketSet::new(&mut SOCKETS.init([SocketStorage::EMPTY; 3])[..]);
    ...
    let http = sockets.add(smoltcp::socket::tcp::Socket::new(
        smoltcp::socket::tcp::SocketBuffer::new(&mut HTTP_RX.init([0; 8192])[..]),
        smoltcp::socket::tcp::SocketBuffer::new(&mut HTTP_TX.init([0; 1024])[..]),
    ));
...
impl Net {
    ...
    /// Lend the live stack to the scry HTTP client. `None` before a lease.
    ///
    /// Everything smoltcp needs travels together on purpose: iface, device and
    /// sockets are one borrow-group, and handing them out separately is how the
    /// borrow checker teaches this lesson the long way.
    pub fn with_stack<R>(
        &mut self,
        f: impl FnOnce(&mut SmolIface, &mut SmolWifiDevice, &mut SocketSet<'static>, SocketHandle) -> R,
    ) -> Option<R> {
        let live = self.live.as_mut()?;
        if self.state != Link::Up {
            return None;
        }
        Some(f(&mut live.iface, &mut live.device, &mut live.sockets, live.http))
    }
