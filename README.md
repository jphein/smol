...
| **The Bard** | a real transformer LLM (260K-param TinyStories, int8, XIP from flash) that **writes a fresh story on demand, fully on-device** — typewriter reveal on the 72×40 OLED, every node with its own protagonist (id8 tells owl tales), and the opening settable per node from the HA dashboard over CFG-`T`, no reflash (#303). Only ~5 lines × ~15 chars are on the glass at once, so the screen is a **window the story moves past**. 🟡 **Not yet on glass:** a sliding-window KV cache so a tale can run without end, plus `inf`/`page` delivery + typewriter pace over CFG-`V` (#302) | 🟢 **on glass** (#300) — bench + canary verified on id8: 67-token story, 202 ms/tok, port proven **bit-for-bit** vs an independent Python reference. #303 merged + hardware-verified (fleet not rolled). #302 is committed and host-tested (36 host tests) but the design spec still reads **"not yet on glass"** — no bench run |
