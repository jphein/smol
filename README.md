...
| **HA Batt** | Home Assistant battery voltages (solar bank + EV pack) on **every** display — the gateway pulls a retained payload from HA's MQTT broker on its WiFi burst and re-broadcasts a `SMOLv1 BATT` mesh frame so its neighbour leaves show it too (single-hop) | **in code · compile-verified (MQTT-native)** — pending hardware flash |
