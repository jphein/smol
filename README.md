...
| **The Bard** | a real transformer LLM (260K-param TinyStories, int8, XIP from flash) **writing stories that never end, fully on-device** — a sliding-window KV cache means it just keeps narrating, typewriter-style, on the 72×40 OLED; every node has its own protagonist (id8 tells owl tales), and the pace + `inf`/`page` delivery are settable from HA with no reflash | 🟢 **on glass** — bench + canary verified, port proven bit-for-bit vs an independent reference (#300); endless narration + delivery control host-verified, bench pending (#302) |
