...
| **OTA updates** | over-the-air firmware via a WiFi burst — dual A/B slots, **canary-one-board-at-a-time** with **app-side rollback**, a retained `smol/ota/announce`, sigil **version names** | **built + deployed** — engine + `ota_publish.sh` + HA panel (bootloader **slot-selects** otadata, but **revert-on-boot-fail is OFF** → canary is the mass-brick defense; A/B fits 4 MB @ ~3.3× headroom) · version splash **live on glass** · **end-to-end canary hardware-verify-pending** (offer-surfacing fix landed; canary next) (#6) |
