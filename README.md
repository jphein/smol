...
| **OTA updates** | over-the-air firmware via a WiFi burst — dual A/B slots, **canary-one-board-at-a-time** with **app-side rollback**, a retained `smol/ota/announce`, sigil **version names** | **spec'd + hardware-de-risked** (bootloader **slot-selects** otadata, but **revert-on-boot-fail is OFF** → canary is the mass-brick defense; A/B fits 4 MB @ ~3.3× headroom) · version splash **live on glass** · build waves next (#6) |
