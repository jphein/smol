...
Mesh time-sync (loop-free, newest-NTP-wins) and **magical realm names** — a pure function of the node id, so id8 is *Eldritch Jewel* and id5 *Obsidian Aegis* (via [realm-sigil](https://github.com/jphein/realm-sigil), corpus from [lexicon](https://github.com/jphein/lexicon.realm.watch)'s `fleet` group; **uniqueness across all 256 ids is a compile-time guarantee** — a colliding namespace does not build). The mapping is derivable; the id↔board *assignment* is not — see [BUILDING.md](docs/BUILDING.md) run under all of it; the boot splash shows the sigil version name.
