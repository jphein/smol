...
| **Relay bridge** | single-hop ESP-NOW→WiFi telemetry to the internet (not browsing — 250 B MTU) | collector committed · gateway **final leg** (E2E leaf→collector unproven) (issue #3) |
| **OTA updates** | over-the-air firmware via a WiFi burst, dual-slot + rollback, sigil **version names** | spike **de-risked** · version splash **live on glass** (issue #6) |
...
A shared-world **MMO snake** (issue #5) just **landed and committed**: a 256×256 toroidal world, a scrolling viewport with no walls, 18-byte state frames at 5 Hz, dead-reckoned peers drawn by their magical name, and a mesh leaderboard (score = length). It — with six collectible **treasure-powers** (Wraith Veil, Zephyr Rune, Aegis Ward, Midas Sigil, Mothlight Lantern, Phoenix Ember) on top — is committed and, as of waves 3+4, **flashed and running on all three boards** (phasing fix `877b2af`, tested 52/52); live multiplayer verification is the next step. The boot splash now shows the sigil VERSION name on real glass: `smol id7 "Draconic Dominion" · build 34 "Pressed Jig" (652155b)`. A [player guide](docs/mesh-snake.md) covers the world, the powers and the leaderboard. Also in flight: the **ESP-NOW→internet relay** (issue #3) — its LAN collector is committed (19 tests) and the gateway is on its final leg (wave-3 fixed a review-predicted DHCP budget on hardware; a wave-4 drain race is in fix; end-to-end leaf→collector isn't proven yet); **OTA updates** (issue #6 — the bootloader spike de-risked the path, and the version-name splash is already live); and a **plugin framework** (issue #7, atomic mode-switch in progress) with a **mesh-view** for the benchmark (issue #8). See `docs/` and the scratch notes for the netcode, gateway, and OTA analysis.
