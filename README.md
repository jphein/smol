...
- **Keyed-CFG channel (#56) — the whole config family.** One `SMOLv1 CFG <id><KEY><value>` frame carries every per-node knob over the mesh, all editable from the HA dashboard, no reflash: **default screen** (#21, key `S`), **LED** mode (#48, `L`), **display units** °F/°C + 12/24h (#43, `U`), **plugin visibility** (#55, `P`), **Custom screen** layout (#45, `Y`), **WiFi slot** switch (#100, `N`), **broker** override (#100, `B`), **OTA-host** override (#100, `O`), **remote reboot** (#52, `R`, transient), **WiFi scan** trigger (#71, `W`, transient). Most apply live; `N`/`B` edge-trigger a reboot; `R`/`W` are one-shot and never cached. ✅ merged.
- **Un-brickable runtime networking (#100).** Switch a node's WiFi slot, MQTT broker leg, or OTA-image host from the HA dashboard — persisted in a 28-byte NVS net record — with an **auto-revert fallback**: a slot/broker that can't connect flips back to the baked-in default on its own, so a bad network change can never strand a board. ✅ merged (Stage 1b–3).
- **Per-node observability (#70/#49/#74).** A retained DIAG record per node: uptime, boot-count, reset-reason, boot-slot, last-OTA-outcome, heap, flush/verify counters, link-quality, time-sync, **network state** (`net=`/`brk=`/`otah=`), and the **applied-config echo** (`cfg=`) that HA compares against its command topics to flag **config drift** — plus a **display-mirror** (the gateway OLED as a live HA image) — so a silent rollback or a drifted node is visible in HA at a glance. ✅ merged.
