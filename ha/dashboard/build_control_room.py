...
def audit_views(cfg, view, prev, extras, st):
    ...
    Each `audited` row is (label, how, entity count, IS ON THE DASHBOARD). That last flag exists so
    the coverage line counts real views only: with no Control Room yet the built view is audited too,
    and folding it into the total printed the memorable nonsense 'VIEWS AUDITED · 2 of 1'."""
    sources, audited = [], []
    for i, v in enumerate(cfg.get("views") or []):
        ...
        if prev is not None and (v is prev or v.get("path") == VIEW_PATH):
            ...
            audited.append((label, "built + preserved live-only (generated here)",
                            len(set(b) | set(x)), True))
        else:
            r = view_entities(v)
            sources.append((f"view:{label}", r))
            audited.append((label, "live cards (not generated by this repo)", len(r), True))
    if prev is None:
        ...
        audited.append((VIEW_PATH, "built — a real run would ADD this view", len(b), False))
    # Coverage is checked, not asserted in prose, and checked as EQUALITY: one audited row per view
    # on the dashboard, no more and no fewer. `>=` would have been satisfied by auditing one view
    # twice, which is the sort of accounting that let #340 read as complete for two releases.
...
def report_check(cfg, view, prev, extras, retired, retiring, st):
    ...
    print(f"\nVIEWS AUDITED · {on_dash} of {len(cfg.get('views') or [])} view(s) on '{DASH}'"
          f" — adding a view enrolls it here automatically (#340)")
    for label, how, n, real in audited:
        print(f"    {'-' if real else '+'} {label:<26} {n:>3} wired "
              f"entit{'y ' if n == 1 else 'ies'} · {how}")
    if not prev:
    ...
    if dead:
        print(f"\nDEAD ROWS · {len(dead)} entit{'y' if len(dead)==1 else 'ies'} wired to a card "
              f"but ABSENT from HA — across all {on_dash} view(s) on '{DASH}'.")
