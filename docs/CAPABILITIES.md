...
| **IMU** | ❌ | ❌ | ❌ no QMI8658 fitted on the ES3C28P (absent from `BOARD.md`'s pin map). ⚠️ **`[IMU] OK` on this board is a vacuous log line** — init is ungated and `let _ =` swallows the NACK, so the boot log reports success for a peripheral that does not exist (#480) | ❌ no `has-imu`; the tilt-driven `Maze` tile is correctly absent | ✅ QMI8658 6-axis with a **hardware pedometer engine** that keeps counting while the IMU is idle; wrist-raise wake | ❌ no IMU chip |
...
| **Battery / fuel gauge** | ❌ | ❌ | 🛠 no PMU, but the board **has** a battery ADC — GPIO9 with a 2:1 divider. Would kill the 0 % cosmetic honestly (#479) | 🛠 `HAS_BATT_ADC = true`, `BATT_ADC_GPIO = 9` declared as of #470 with **no reader** — the only reference outside the board file is a comment warning that binding a button there samples ~2 V (#479) | ✅ AXP2101 — charger profile, power-key latch, battery monitoring, live per-subsystem current estimation | 🛠 `HAS_BATT_ADC = false`, commented *"none **known** … USB-powered bench board"* — none known is not none possible (#486) |
...
| Target | Issues |
|---|---|
| **s3-cyd** (parent #398) | #476 fleet audio path · #477 audio-out first listen · #478 audio-in phase 2 · #479 battery ADC reader · #480 the vacuous `[IMU] OK` · #481 light sleep · #482 backlight dimming · #483 Cast mirror blank · #484 the xtensa high-water instrument |
