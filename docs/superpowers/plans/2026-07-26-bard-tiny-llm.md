...
- [ ] **Step 9.3:** Registration — the six sites plus wire arms, each `#[cfg(feature = "bard")]`, exactly per the Sigil template:
  ...
  - `plugin_bit` → `None` arm; `from_wire`/`as_wire`/`live_screen`: **the wire mapping is STRING-keyed, not numeric** (corrected at T9) — `as_wire` returns tokens (`"Menu"`, `"Clock"`, …) and `from_wire` parses them; Bard's token is `"Bard"` (app.rs:289/:327), arm cfg-gated per the **WledRemote** precedent, which keeps `as_wire` total in every cfg combination. There is no numeric id to allocate.
...
- Render: `FONT_5X8`, 14 cols × 5 rows. Word-wrap `STORY_TEXT[..shown]` (greedy break at spaces, hard-break 14), keep the **last 5** lines (single back-to-front scan — no line ring buffer). While `Composing`, draw a `'|'`/`' '` alternating quill at the cursor. On `Told`, last line gets `~ fin ~` centered. Token-cap truncation is the **NORMAL** ending, not an edge case (T8 measured: 19/20 seeds ran to the 220-token budget; ~5% end naturally — the EOS path is real and reachable, never dead code) — `Story` reports it via `Done { truncated }`, and T10 appends `…` at the cut so the hard stop reads as intentional trailing-off. Never wait for a graceful ending or assume the last line is complete.
