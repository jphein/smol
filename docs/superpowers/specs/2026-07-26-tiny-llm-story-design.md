...
> **§5 RAM budget — the `SEQ_CAP` knob fired.** At `SEQ_CAP=256` the canonical fleet image (`espnow,cast,io,bard`) **fails to link** (`.bss` overflow by 20,704 B in DRAM). Shipped at **`SEQ_CAP=192`**. Measured loadable-section deltas (bard vs no-bard): `.bss` +78,528 B · `.data` +1,184 B · `.rodata` +285,360 B (283,096 = the blob). **Remaining DRAM headroom ≈ 2.6 KB** (bracketed empirically: +2,000 B links, +4,000 B overflows) — anything added to this tier must verify the LINK, not the compile; `SEQ_CAP=160` is the next relief (~14 KB, ~145-token stories). Story length is now **cache-bound at ~177 generated tokens (~360 chars)** with a ~15-token prompt — the token budget (220) is currently unreachable. Goldens regenerated at 178 tokens, still bit-for-bit across both implementations.
>
