...
| **Keyed-CFG channel — the whole remote-config family** — one `SMOLv1 CFG <id><KEY><value>` frame carries every per-node knob over the mesh, all editable from the HA dashboard, no reflash: `S` screen · `L` LED · `U` units · `P` plugins · `Y` custom screen · `N`/`B`/`O` network identity (un-brickable auto-revert) · `R` reboot · `W` scan · `G`/`g` IO pin-map · `T` Bard prompt | #56 + #21/#48/#43/#55/#45/#100/#52/#71/#72/#303 | 🟢 byte-accurate wire in [protocol.md](protocol.md#cfg--keyed-per-node-config-channel-56); most keys apply live, `N`/`B` edge-trigger a reboot, `R`/`W` are one-shot and never cached |
