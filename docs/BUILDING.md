...
- **Secrets:** real WiFi creds **and the #190 `GROUP_KEY`** (the fleet-shared mesh-auth HMAC key, `espnow` builds) live only in git-ignored `rust/clock/src/secrets.rs` (the repo is public). Every board must share one `GROUP_KEY` + `GROUP_KEY_EPOCH`, or a MAC-mismatch drops its frames (in observe it only bumps a DIAG counter; in a future enforce release it partitions). **Since #471 it matters WHICH counter**, and the common misprovisioning is the loud one: a board with the right `GROUP_KEY_EPOCH` and the wrong `GROUP_KEY` bumps **`mfk=`** — the *actionable* counter, expected to be zero on a healthy fleet — so a mis-keyed board announces itself instead of hiding in the benign `mf=` traffic that legitimately un-MAC'd members generate forever. A wrong *epoch* bumps `mf=` and is correspondingly quiet. ⚠️ `GROUP_KEY_EPOCH` is **not a free choice**: it must not be `0` or printable ASCII (`0x20..=0x7E`), because SMOLv1 payloads are ASCII and such an epoch lets ordinary un-MAC'd frames land in `mfk=` and impersonate a forgery. A compile-time assert enforces it, so a bad value is a build failure rather than a fleet-wide false alarm — see `net/wire.rs`'s `group_epoch_is_unambiguous`.
...
The mesh wire protocol — exact byte layouts, cadence and per-frame verification status — is documented in **[docs/protocol.md](protocol.md)**: HELLO/ACK, BEACON, TIME, BATT/GRID, **CFG**, **DIAG**, **SCAN**, RELAY/RELAYACK, **UP2/RELAYACK2** (routed multi-hop, #13/#124), **BATT2/GRID2**, SNK (**shipped**, not design-stage — #5 closed 2026-07-08), **FAM** (the Familiar, #57), **ELECT** (#278/#269) and the **leaf mesh-OTA** frames (#40). Treat protocol.md as the list; enumerating frames here just goes stale.
