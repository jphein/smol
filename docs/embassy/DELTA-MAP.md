...
- **Donor-sync item (2026-08-25, via #328 triage): `experiments/mesh_elect_verify` is clippy-red on main** at `consensus.rs:24` and `wire_tests.rs:42` — both in donor-verbatim files, and `gate.sh` clippies only the rust/clock tiers, so nothing has ever linted them. Silencing the donor's assertion is a cross-repo decision: fix it in the donor (dream/feat-embassy reference) and re-vendor, or accept the red as the price of byte-diffability. Do NOT "clean it up" locally — that breaks the byte-diff against the donor that the verbatim policy exists for.
